...
## 2026-08-20 - V1.5 工程收尾

- 清理全部 18 条 ESLint warning，并在 CI 中启用 `--max-warnings=0` 防止回归。
- 修复通话页连接延迟定时器在卸载后仍可能触发的问题，统一清理连接、时长与画面分析定时器。
- Hook 回调改为读取最新设置，避免自动问候、Debug Now 和模型配置持有旧状态。
- 移除未使用的旧首页组件、尺寸计算与类型声明。
- 完整 `npm run verify`、Web 生产构建和浏览器像素验收通过；三位角色素材请求均为 200，控制台 0 error。

## 2026-08-19 - V1.5 工程稳定性补强

### 数据与队列

- 修复旧聊天快照覆盖当前消息状态的恢复优先级缺陷。
- SQLite 改为增量 upsert，聊天写入串行化，文件快照节流并采用临时文件替换。
- 用户消息先写入持久发送队列；异常退出后可恢复，成功/失败沿用同一消息 id。
- 主聊天请求增加 45 秒超时和页面退出取消机制。

### 媒体与备份

- 相册图片复制到应用媒体目录，远端请求时转换为多模态接口可读取的 data URL。
- 消息图片增加磁盘/内存缓存和可见失败占位。
- 备份升级到 schema v2，包含媒体、完整性校验、恢复标记和中断自动回滚，同时继续读取 v1。
- 数据管理支持从 Files 导入 JSON 和分享最近备份，最多保留 20 份本地导出。

### 工具链与性能

- 提交 `package-lock.json`，增加 ESLint、Vitest、GitHub Actions 和统一 `npm run verify`。
- Expo SDK 54 依赖对齐到兼容补丁版本；移除仓库内无关的远程 shell 安装脚本。
- 使用系统中英文字体，首屏不再等待六份大型 CJK 字体；Web 构建从约 141 MB 降至约 68 MB。
- 新增应用级错误边界；普通用户模式停止记录包含原始对话内容的 Debug Trace。

### 验证

- 类型检查、lint、6 个单元测试、聊天恢复、Debug Now、90 项视觉资产、V1.5 和工程加固检查通过。
- Expo Web 构建成功；浏览器首屏三个角色素材均返回 200，控制台 0 error。
- 未启动 Simulator；截图证据保存在本地 `output/playwright/v1.5-engineering-home.png`。

## 2026-08-11 - V1.5 可信伴侣与角色创作工作台

### 概述

完成 V1.1—V1.5 的核心闭环：可靠配置与诊断、可控长期记忆、关系成长时间线、自定义角色创作、角色体检与版本回退。

### 主要能力

- Onboarding 在保存前验证文字模型连接，API Key 默认隐藏。
- 聊天持久化继续兼容 SQLite、索引记录、旧键和文件快照。
- 长期记忆记录来源消息、可信度和状态，并支持去重、修正、锁定和删除。
- 关系按照“初次相识—逐渐熟悉—彼此信任—共同日常”推进，保存已确认里程碑。
- 角色创作工作台支持自定义角色、简单/专家模式、离线角色体检和设定版本回退。
- 新增本地备份/恢复、恢复前自动快照和错误诊断；API Key 不写入导出文件。
- 移除语音通话中的模拟转写，缺少 Speech-to-Text 时展示真实配置状态。
- 日记只根据真实消息生成，不虚构情绪或关系进度。

### 数据兼容

- 不更改既有聊天存储键；恢复走统一聊天持久化层。
- 角色设定版本与聊天、记忆、日记、纪念日和关系状态隔离。
- 视觉素材继续由集中注册表和本地 fallback 管理。

### 验证

```bash
npx tsc --noEmit
npm run verify:chat-history
...
### 历史说明
这些早期人设图后来已由新版角色素材替换，当前运行时和展示站统一走 v2 角色素材注册表。
